...
def copy_stripped_T1(args,subdir_names):
    ...
    if not os.path.exists(brainmask):
        ...
        cmd='mri_convert %s --reslice_like %s/highres001.nii.gz  %s/highres001_brain.nii'%(brainmask,
...
def convert_dicom_to_nifti(args, subdir):
	...
	for f in boldfiles:
		...
		if not os.path.exists(rundir):
			os.mkdir(rundir)
		cmd='mv %s %s/bold_orig.nii.gz'%(os.path.join(subdir_names['BOLD'],f),rundir)
		print cmd
		#if rundir.find("SC")==1:
		 #cmd='fslroi %s/bold_orig.nii.gz %s/bold.nii.gz 5 212'%(rundir,rundir)
		 #print cmd
		#else:
		 #cmd='fslroi %s/bold_orig.nii.gz %s/bold.nii.gz 5 180'%(rundir,rundir)
		 #print cmd	
		if not args['testmode']:
