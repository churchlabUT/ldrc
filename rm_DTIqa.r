...
filenames = c(Sys.glob(sprintf('%sldrc_?_[0-9][0-9][0-9]', dir)), Sys.glob(sprintf('%sldrc_*second', dir)), Sys.glob(sprintf('%sldrc_*third', dir)), Sys.glob(sprintf('%sH_LD*_?_second', dir)), Sys.glob(sprintf('%sH_LD*_?_third', dir)) )
...
DTI.setup = file('/corral-repl/utexas/ldrc/SCRIPTS/launch_dtiqa.txt','w')
...
rewrite = scan('/corral-repl/utexas/ldrc/SCRIPTS/launch_dtiqa.txt', what = 'character', sep = '\n', blank.lines.skip=T)
DTI.setup = file('/corral-repl/utexas/ldrc/SCRIPTS/launch_dtiqa.txt','w')
