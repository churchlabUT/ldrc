...
# READ IN DATA
  ...
  for (i in 1:length(group)){ 
    ...
    # SUB
    for (sub in sub.dirs){
      bold.dirs = c(Sys.glob(sprintf("%s/BOLD/*SST*", sub)))
...
# SAVE ALL DATA FRAMES TO CSVs
    ...
    write.csv(out.scrub.all_1, file=sprintf("%s/lev1_QC/SST/A_out_scrub_1.csv", wd), na="NA", row.names=FALSE)
    write.csv(out.scrub.all_2, file=sprintf("%s/lev1_QC/SST/A_out_scrub_2.csv", wd), na="NA", row.names=FALSE)
    write.csv(out.scrub.all_3, file=sprintf("%s/lev1_QC/SST/A_out_scrub_3.csv", wd), na="NA", row.names=FALSE)
    write.csv(out.scrub.all_4, file=sprintf("%s/lev1_QC/SST/H_out_scrub_2.csv", wd), na="NA", row.names=FALSE)
    write.csv(out.scrub.all_5, file=sprintf("%s/lev1_QC/SST/H_out_scrub_3.csv", wd), na="NA", row.names=FALSE)
    write.csv(out.scrub.all_6, file=sprintf("%s/lev1_QC/SST/A_out_scrub_c.csv", wd), na="NA", row.names=FALSE)
    write.csv(out.scrub.all_7, file=sprintf("%s/lev1_QC/SST/H_out_scrub_c_2.csv", wd), na="NA", row.names=FALSE)
    write.csv(out.scrub.all_8, file=sprintf("%s/lev1_QC/SST/H_out_scrub_c_3.csv", wd), na="NA", row.names=FALSE)
