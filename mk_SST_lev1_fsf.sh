...
do
  ...
  do
    ...
    sed   -e 's:NVOLS:'${nvols}':g'  -e 's:SUBDIRPATH:'${sub}':'  -e 's:RUNDIRPATH:'${run}':' -e 's:RUNNUM:'${runnum}':'  /corral-repl/utexas/ldrc/SCRIPTS/Feat_Templates/new_design_sst_template.fsf > ${sub}/model/SST/designs/run_${runnum}.fsf
...
do
  ...
  do
    ...
    sed   -e 's:NVOLS:'${nvols}':g'  -e 's:SUBDIRPATH:'${subname}':'  -e 's:RUNDIRPATH:'${run}':' -e 's:RUNNUM:'${runnum}':'  /corral-repl/utexas/ldrc/SCRIPTS/Feat_Templates/new_design_sst_template.fsf > ${subname}/model/SST/designs/run_${runnum}.fsf
