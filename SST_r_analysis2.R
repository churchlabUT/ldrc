...
#example code for reading in R-friendly versions of behav data and running analyses
...
#To compare first and second mri sessions

group=c("ldrc_[0-9]_[0-9][0-9][0-9]","ldrc_*second", "PHILIPS/H_LD*[0-9]", "H_LD*_[0-9]","ldrc_c_[0-9][0-9][0-9]")

chars=c(5,12,11,13,5)
...
#create a complete data set with all subjects.
rm("dat_all")

for (sub in subdirs){
  behav_dirs=Sys.glob(sprintf("%s/behav/SST/SST_R*", sub))
  subnum=substrRight(sub, chars[i])
  #read in each data set and create a sub indicator and run indicator
  for (run in behav_dirs){
     run_num=substrRight(run,1)

     #This is an iffy chunck of code, since it assumes there is only a single file that ends in R.txt
     ...
     # if it doesn't already exist, I need to create dat_all
     if (exists("dat_all")==FALSE){
       dat_all=dat_loop } else
...
#create a variable that picks out the first occurrence of a trial num (first of repeats)

#this will be 1 too long
...
####### SET LOWER CUTOFFS, SET TO 0 FOR NO CUTOFFS 
...
#see what variables are in the data fram
names(dat_all)

#see what subjects are here (top # is subind and number below is # of observations in data set
table(dat_all$subind)
...
for (k in 1:sessions){
	...
	sess_1_subs=names(table(dat_all$subind))
	sess_1_subs_type=substrLeft(names(table(dat_all$subind)),1)
	...
	sess_2_subs=substrLeft(names(table(dat_all$subind)),5)
	sess_2_subs_type=substrLeft(names(table(dat_all$subind)),1)
	...
	H_sess_1_subs=names(table(dat_all$subind))
	...
	H_sess_2_subs=substrLeft(names(table(dat_all$subind)),11)
	...
	control_subs=names(table(dat_all$subind))
	control_subs_type=substrLeft(names(table(dat_all$subind)),1)
...
### AVG ACROSS ALL SUBJS PER SESSION TYPE


after_stop=data.frame(matrix(0,nrow=length(sub_nums), ncol=length(run_nums) ))

total_runs=sum(table(dat_all$runnum))/128
...
vec_num=length(dat_all$correct[dat_all$subind==sub_nums[i]  & dat_all$run==run_nums[j]])
...
for (b in 1:vec_pos_stop_num){
	...
	total_num_go_trials_after_stop_RT[RT_pos[b]:RT_pos[b]]=dat_all$RT[go_pos_after_stop]
	...
	if (total_num_go_trials_after_stop[b]!=0){
...
vec_stop_mask=dat_all$correct==4
vec_go_mask=dat_all$correct==1
