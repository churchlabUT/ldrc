...
# READ IN DATA
  ...
  group = c("H_LD*_[0-9]_second", "H_LD*_[0-9]_third", "LDFHO*_[0-9]", "H_LD*_c_second", "H_LD*_c_third")
  chars = c(13,12,7,13,12)
  ...
  for (i in 1:length(group)){ 
    ...
    # SUB
    for (sub in sub.dirs){
      bold.dirs = c(Sys.glob(sprintf("%s/BOLD/run*_S*_*", sub)))
      subnum = substrRight(sub, chars[i])

	# RUN
        for (run in bold.dirs){
          run.num = substrRight(run,1)
          scrub.file = Sys.glob(sprintf("%s/scrub_files/fd_confounds_drop5.txt", run))
	    ...
	    next
          ...
          scrub.loop = read.table(scrub.file)
